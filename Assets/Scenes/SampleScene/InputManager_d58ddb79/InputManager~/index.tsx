...
function App() {
    ...
    const handlePhaseChange = () => {
        pmInput.GamePhase = gamePhase === 0 ? 1 : 0;
                    ...
                    {objType === 0 ? (
                        <>
                            ...
                            <MenuButton emoji="🏗️" x={mouseX - 110} y={mouseY - 110} isBlocked={isPathBlocked} 
                                onClick={() => !isPathBlocked &&pmInput.BuildTower(0)} />
                            <MenuButton emoji="💣" x={mouseX + 30} y={mouseY - 110} isBlocked={isPathBlocked}
                                onClick={() => !isPathBlocked &&pmInput.BuildTower(1)} />
                            <MenuButton emoji="ℹ️" x={mouseX - 110} y={mouseY + 30} isBlocked={isPathBlocked}
                                onClick={() => !isPathBlocked &&pmInput.BuildTower(2)} />
                            <MenuButton emoji="❌" x={mouseX + 30} y={mouseY + 30} isBlocked={isPathBlocked}
                                onClick={() => !isPathBlocked &&pmInput.BuildTower(3)} />
                        </>
                    ) : (
                        objType > 1 ? ( // 0(빈칸), 1(목표)이 아닐 때만 노출
                            <MenuButton emoji="🗑️" x={mouseX - 40} y={mouseY - 40} onClick={() => pmInput.RemoveObject()} />
                        ) : null
                    )}
                </>
            )}
